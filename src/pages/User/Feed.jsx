    ...
    <div className="min-h-screen bg-[#F5F5DC] font-['Roboto']">
      ...
      <header className="bg-[#FFB74D] p-4 flex items-center justify-between fixed top-0 w-full z-20 shadow-md">
        ...
        <div className="flex flex-col items-center md:hidden w-full">
          ...
          <div className="flex justify-between w-full px-4">
            <HiOutlineHome className="text-black text-2xl cursor-pointer" onClick={() => navigate("/feed")} />
      ...
      >
        <div className="flex flex-wrap justify-center gap-2 md:space-x-8">
          ...
          <button className="bg-green-500 text-white px-6 py-2 rounded-lg" onClick={() => navigate("/challenges")}>
            Retos Activos
          </button>
        </div>
      </div>
        ...
        {recipes.map((recipe) => (
          ...
          >
            ...
            <div className="p-4 flex justify-between items-center border-t">
              ...
              <button
                className={`flex items-center space-x-2 ${savedRecipes[recipe.id] ? "text-orange-500" : "text-gray-500"}`}
                onClick={() => toggleSave(recipe.id)}
              >
                <HiOutlineBookmark className="text-lg" />
                <span className="text-sm">{savedRecipes[recipe.id] ? "Guardado" : "Guardar"}</span>
              </button>
            </div>
          </div>
        ))}
